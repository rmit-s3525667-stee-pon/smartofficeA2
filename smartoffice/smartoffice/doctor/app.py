...
def all_appointments():
    ...
    if request.method == 'GET':
        appointments = api_caller.get_appointments_by_doctor(
            int(session['id']))
        show_individual = None
    elif request.method == 'POST':
        input_date = request.form['input_date']
        show_individual = True
        appointments = api_caller.get_appointments_by_doctor_and_date(
            int(session['id']), input_date)
...
def add_availability():
    ...
    if end_of_this_week < date <= end_of_next_week:
        event_id = api_caller.add_to_calendar(
            summary, doctor_id, date, time_start, time_end, calendar_id)
        api_caller.add_availability(
            doctor_id, date, time_start, time_end, event_id)
        add_appointment_automatically(
            doctor_id, date, time_start, time_end, calendar_id)
...
def add_appointment_automatically(doctor_id, date, time_start, time_end, calendar_id):
    ...
    for x in range(num):
        ...
        event_id = api_caller.add_to_calendar(
            summary, doctor_id, date, time_start, time_end_format, calendar_id)
        api_caller.add_appointment(
            doctor_id, date, time_start, time_end_format, patient_id, event_id)
...
def remove_appoinment_automatically(calendar_id, doctor_id, date, time_start):
    ...
    appointments_dumps = json.dumps(
        appointments, default=lambda o: o.__dict__, sort_keys=True, indent=4)
        ...
        if app['doctor_id'] == int(doctor_id):
            if app['date'] == date:
                if app['time_start'] == time_start_format:
                    ...
                    api_caller.remove_from_calendar(
                        calendar_id, app['event_id'])
                    st = st + datetime.timedelta(minutes=15)
                    time_start_format = datetime.datetime.strftime(
                        st, '%H:%M:%S')
